...
function get_all_students()
{
    global $conn;
    connect_db();
    $sql = "select * from sinhvien where role = 0 order by username";
...
function edit_student($currentName, $username,$password, $name, $email, $phone)
{
    global $conn;
    connect_db();
...
function checkUsername($username){
    ...
    connect_db();
    $username = mysqli_real_escape_string($conn, htmlentities($username));
    $sql = "select * from sinhvien where role = 0 and username = '$username' ";
    $query = mysqli_query($conn, $sql);
    $result = mysqli_fetch_assoc($query);
    if(!empty($result)){
        return TRUE;
